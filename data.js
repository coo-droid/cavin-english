...
// =====================================================
// v17.1 WRITING_PROMPTS — Writing Review 用お題（CAVIN/咲品文脈）
// category: 'client' | 'daily'
// =====================================================
const WRITING_PROMPTS = [
  // ===== クライアント対応 (商談/メール/雑談) =====
  { category: 'client', icon: '💼', title: 'ジャカルタHNWIへフォロー',
    situation: '先週ジャカルタで会ったHNWIクライアントに、フォローアップのメールを書く。',
    hint: 'Thanks → 印象的だった話 → 次の提案を軽く投げる' },
  { category: 'client', icon: '🌸', title: 'CAVINを1分で紹介',
    situation: '新しいクライアントに「CAVINって何やってる会社？」と聞かれた。1分で答える。',
    hint: 'What → Why different → Why now' },
  { category: 'client', icon: '📦', title: '配送遅延を品よく謝罪',
    situation: 'クライアント向けの花が物流の問題で1日遅れる。状況説明 + 補償案。',
    hint: '即謝罪 → 原因簡潔 → 補償 → 再発防止' },
  { category: 'client', icon: '💸', title: '価格が高いと言われた時',
    situation: '「他より高い」と言われた。価格ではなく価値で返す。',
    hint: 'Acknowledge → Reframe value → Specific example' },
  { category: 'client', icon: '💐', title: '咲品ブーケを紹介',
    situation: '咲品(Sakihin)を新規法人クライアントに紹介。国産高品質ブーケのこだわりとストーリー。',
    hint: 'Origin story → Quality proof → Use case' },
  { category: 'client', icon: '📱', title: 'WhatsAppで近況返信',
    situation: 'インドネシアのクライアントから写真と「How was your week?」が来た。',
    hint: '自然な近況 → 相手に乗る → 来月の予定軽く触れる' },
  { category: 'client', icon: '🎁', title: 'カスタムオーダーをヒアリング',
    situation: 'クライアントの誕生日用に特別なアレンジ。好みを丁寧に聞き出す。',
    hint: 'Occasion → Preference → Budget → Timing' },
  { category: 'client', icon: '🌏', title: '日本の花の季節性を説明',
    situation: '「なぜ今この花が手に入らないの？」と聞かれた。旬を文化として語る。',
    hint: '日本では旬を尊ぶ → 旬の花のほうが美しい → 待つ価値' },
  { category: 'client', icon: '✉️', title: '紹介をお願いするメール',
    situation: '満足してくれているクライアントに、彼の友人を紹介してほしいと頼む。',
    hint: '感謝 → どんな人を探してる → 紹介の重さを軽くする' },
  { category: 'client', icon: '🗣', title: 'クレームに即返答',
    situation: '「届いた花の発色が写真と違う」と言われた。10秒で対応開始を示す。',
    hint: '受け止め → 写真を要求 → 解決策2つ' },
  { category: 'client', icon: '🍷', title: 'ディナーで関係構築',
    situation: '会食で「日本のビジネスは難しいって聞くけど？」と聞かれた。',
    hint: '正直に → 自分の視点 → 相手の国の話に振る' },
  { category: 'client', icon: '🎉', title: '大口契約のクロージング',
    situation: '長期パートナーシップの最後の一押し。Yes/No/紹介の3択を品よく出す。',
    hint: 'If yes → If no → Referral ask' },

  // ===== 日常ビジネス英会話 =====
  { category: 'daily', icon: '✈️', title: '空港で迎えのドライバーに',
    situation: 'ジャカルタ着、迎えのドライバーに自己紹介+ホテルまでの軽い雑談。',
    hint: 'Greeting → Thanks → 軽い質問' },
  { category: 'daily', icon: '🍽', title: '会食前のスモールトーク',
    situation: '初対面のクライアントとディナー。料理が来る前の5分を埋める。',
    hint: '土地への興味 → 自分の話を少し → 相手に振る' },
  { category: 'daily', icon: '☕', title: 'コーヒーブレイクで近況',
    situation: '取引先と会議の合間。「ここ最近どう？」に答える。',
    hint: '具体的な近況1つ → ポジティブに → 相手に振る' },
  { category: 'daily', icon: '🏨', title: 'ホテルで小さな要望',
    situation: '高層階・静かな部屋・遅めのチェックアウトを品よく頼む。',
    hint: 'お願い口調 → 理由軽く → 感謝先回り' },
  { category: 'daily', icon: '📞', title: '会議の日程変更を頼む',
    situation: '体調不良で明日の会議を翌週にずらしたい。失礼にならないように。',
    hint: '簡潔な理由 → 代案2つ → 迷惑を最小化する姿勢' },
  { category: 'daily', icon: '🎤', title: '業界イベントで初対面',
    situation: 'フラワーバイヤーイベントで知らない人に話しかける。',
    hint: 'きっかけ → 自己紹介 → 軽い質問' },
  { category: 'daily', icon: '💬', title: '雑談から商談へ移行',
    situation: '会食で打ち解けた後、本題に品よく入りたい。',
    hint: 'ブリッジ → 本題前置き → 切り出し' },
  { category: 'daily', icon: '🙏', title: '今は無理と断る',
    situation: '紹介された案件、今は受けられない。関係を傷つけず断る。',
    hint: '感謝 → 状況 → 将来の余地' },
  { category: 'daily', icon: '🌅', title: '今日の気分を1段落',
    situation: '今朝の気分・今日やりたいことを、軽い日記として英語で。',
    hint: 'Free format — 思ったまま' },
  { category: 'daily', icon: '🎯', title: '今週の振り返り',
    situation: '今週うまくいったこと/反省したいことを3-4文で。',
    hint: 'Win → Miss → Next' },
];
